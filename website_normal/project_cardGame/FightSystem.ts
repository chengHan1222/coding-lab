class FightSystem {
    ...
    static isPlayCardMove: boolean = false;
    static objPlayCardFocus: PlayerCard = null;
    static tagPlayCardFocus: HTMLElement = null;
    static objAttacker: PlayerCard = null;
    static objDefender: PlayerCard = null;
    static mapPlayerCards: {[key: string]: PlayerCard} = {};
    static mapSafeSideRange: {[key: string]: number} = {};
    static arrIntPointSite: Array<number> = [];
    static arrIntOriginSite: Array<number> = [];


    static startToChooseCard(): void {
    ...
    static changePlayerToChooseCard(): void {
        if (!player1.checkIfHaveEnoughCard()) return;
    
        FightSystem.renewHTMLValue();
    
        player2.createPlayerCardDecks();
    }
    private static renewHTMLValue(): void {
    ...
    static gameInitial(): void {
        if (!player2.checkIfHaveEnoughCard()) return;
    ...
    static startFight(player: Player): void {
        player.displayPlayerFightingCards();

        Player.playerAttacker = player;
        if (player.strName === 'Player 1') Player.playerDefender = player2;
        else Player.playerDefender = player1;
    }


    static putPlayerCardOnGround(): void {
    ...
    static checkIfOutSafeSide(event: { target: { parentElement: { className: string | string[]; id: string | number; }; }; pageX: number; pageY: number; }): boolean {
        if (event.target.parentElement === null ) return true;
