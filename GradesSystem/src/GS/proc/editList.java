...
public class editList {
	...
	private class Controller {
		...
		Controller(Model m, View v){
			...
			switch (model.getIdentity()) {
				case "Professor":
					view.hideComboBox();
					view.showList(model.getLessonList());
					break;
				...
				case "Manager":
					view.cbSetYear.addActionListener(new ActionListener() {
				        ...
				        public void actionPerformed(ActionEvent e) {
				        	...
				        	view.showList(model.getLessonList(semester), semester);
				        }
				    });
					break;
			}
	...
	private class Model {
		...
		String[] getLessonList(){
			return ((Professor) user).getLessonList();
		}
		
		String[] getLessonList(int semester) {
			return Lesson.getLessonListInSemester(semester);
		}
	...
	private class View {
		...
		void showList(String[] data, int semester) {
			...
			for (int index = 0; index < data.length; index++) {
				...
				if (semester == -1) addRow(new Lesson(LessonId));
				else addRow(new Lesson(LessonId, semester));
			}
			
			frame.setVisible(true);
		}
		
		private void addRow(Lesson lesson) {
			JPanel rowPanel = new JPanel();
			rowPanel.setPreferredSize(new Dimension(480, 40));
			rowPanel.setLayout(new BorderLayout(20, 0));
			
			JLabel lb01 = new JLabel(lesson.id + "");
			rowPanel.add(lb01, BorderLayout.WEST);
			JLabel lb02 = new JLabel(lesson.name);
			...
			if (this.openFrame.equals("editScore")) {
				...
				btn.addActionListener(e -> {
					// 開啟編輯畫面
					new editScore(lesson.id, lesson.semester);
				});
			}
			else if (this.openFrame.equals("showLessonInfo")) {
				btn.setText("課程資訊");
				btn.addActionListener(e -> {
					// 開啟編輯畫面
					new showLessonInfo(lesson.id, lesson.semester);
				});
			}
